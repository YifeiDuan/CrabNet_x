...
class CONSTANTS():
    def __init__(self):
        ...
        self.atomic_symbols_cpd = ['None', 'H', 'He', 'Li', 'Be', 'B', 'C', 'N',
                               'O', 'F', 'Ne', 'Na', 'Mg', 'Al', 'Si', 'P',
                               'S', 'Cl', 'Ar', 'K', 'Ca', 'Sc', 'Ti', 'V',
                               'Cr', 'Mn', 'Fe', 'Co', 'Ni', 'Cu', 'Zn', 'Ga',
                               'Ge', 'As', 'Se', 'Br', 'Kr', 'Rb', 'Sr', 'Y',
        ...
        self.idx_symbol_dict_cpd = {(i): sym for
                                i, sym in enumerate(self.atomic_symbols)}

        self.symbol_idx_dict_cpd = {sym: (i) for
                                i, sym in enumerate(self.atomic_symbols)}
...
class EDMDataset(Dataset):
    ...
    def __init__(self, dataset, n_comp, task):
        self.data = dataset
        self.n_comp = n_comp
        self.task = task
    ...
    def __getitem__(self, idx):
        ...
        if self.task == None:
            return {'data': X, 'targets': y, 'formula': formula}
        else:
            return {'data': X, 'targets': y, 'formula': formula, 'tasks': self.task}


def get_edm(path, cpd_token, eos_token, elem_prop='mat2vec', n_elements='infer',
    ...
    #if not inference: #thorben keep duplicates
    #    df = df.groupby(by='formula').mean().reset_index()  # mean of duplicates

    #if not inference: #thorben use 1st 
    df = df.groupby(by='formula').first().reset_index()
    ...
    # check if the target column is a string; for case of MREG
    if df['target'].dtype == object:
        y = np.array(list(df['target'].apply(ast.literal_eval))).astype(data_type_np)
    # case where target is has a single value
    else:
    ...
    if cpd_token:
        cpd_toks = np.ones((elem_num.shape[0], 1, elem_num.shape[2])) * 119
        elem_num = np.concatenate((cpd_toks, elem_num), axis=1)

        cpd_frac = np.zeros((elem_frac.shape[0], 1, elem_frac.shape[2]))
        elem_frac = np.concatenate((cpd_frac, elem_frac), axis=1)

    if eos_token:
        print('EOS Used')

        eos_toks = np.ones((elem_num.shape[0], 1, elem_num.shape[2])) * 120
        elem_num = np.concatenate((elem_num, eos_toks), axis=1)

        eos_frac = np.zeros((elem_frac.shape[0], 1, elem_frac.shape[2]))
        elem_frac = np.concatenate((elem_frac, eos_frac), axis=1)
...
class EDM_CsvLoader():
    ...
    def __init__(self, csv_data, cpd_token=False, eos_token=False, masking=False,
    ...
    def get_dataset(self, path, task_id):
        main_data = list(get_edm(path, self.cpd_token, self.eos_token,
                                 elem_prop='mat2vec',
                                 ...
                                 scale=self.scale))
        self.n_train = len(main_data[0])
        print(len(main_data[0]))
        #self.n_elements = main_data[0].shape[1] // 2
        return EDMDataset(main_data, self.n_elements, task_id)
    ...
    def get_data_loaders(self, inference=False):
        ...
        #check if multitask learning
        if type(self.csv_data) is str:
            dataset = self.get_dataset(self.csv_data, None)
        elif type(self.csv_data) is list:
            dataset_list = []
            for task_i in range(len(self.csv_data)):
                dataset_list.append(self.get_dataset(self.csv_data[task_i], task_i))
