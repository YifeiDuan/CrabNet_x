...
class SaveOutput:
    def __init__(self, N, steps, attn_shape, chunks_shape):
        ...
        ### array_shape = (n_data*steps, 1, num_heads, target_seq_len, source_seq_len)
        ...
        """
        dedicated arrays that specifically record the attn of CPD token to elem tokens
        """
        self.cpd_array_shape = tuple([array_len] + list(attn_shape)[1:3] + [1] + list(attn_shape)[4:])
        ### array_shape = (n_data*steps, 1, num_heads, target_seq_len, source_seq_len)
        self.cpd_attn_arrays = [self.init_array(
            self.roots[layer],
            self.layer_names[layer] + "_CPD",
            self.cpd_array_shape,
            tuple(list(chunks_shape)[:3] + [1] + list(chunks_shape)[4:])) for layer in range(self.N)]  # for each layer
    ...
    def __call__(self, module, module_in, module_out):
        ...
        if model.capture_flag == True:
            ...
            ### module_out[1].shape = (batch_size, num_heads, target_seq_length, source_seq_length)
    ...
    def save_zarr(self):
        for L in range(self.N):
            ...
            ### mod_out (self.outputs[L]) is a list of num_batch (n_data/batch_size) torch tensors for attn weights of the layer
            ###     each of these tensors has a shape = (batch_size, num_heads, target_seq_len, source_seq_len)
            ...
            ### attn_data.shape = (n_data, 1, num_heads, target_seq_len, source_seq_len)
            ...
            """
            cpd_attn_arrays data saving
            """
            cpd_attn_data = attn_data[:, :, :, 0, :]
            ### attn_data.shape = (n_data, 1, num_heads, target_seq_len, source_seq_len)

            if self.cpd_attn_arrays[L].nchunks_initialized == 0:
                # if the array is empty, then fill the first chunk
                self.attn_arrays[L][:n_data] = cpd_attn_data
            elif self.cpd_attn_arrays[L].nchunks_initialized == B * model.epochs:
                # array is filled all the way
                continue
            else:
                # append new chunks continuously
                self.cpd_attn_arrays[L][self.array_count * n_data:
                                    (self.array_count + 1) * n_data] = cpd_attn_data
...
if __name__ == '__main__':
    ...
    for mat_prop in mat_props:
        ...
        if model.eos_token:
            seq_len += 1
        ### if cpd_token is True, seq_len +1 for the additional token; if eos_token is True, seq_len +1 for yet another token

        chunks_shape = [n_data, 1, H, seq_len, seq_len]
        chunks_shape = tuple(chunks_shape)
        attn_shape = [n_data, 1, H, seq_len, seq_len]
        ...
        if capture_every == 'epoch':
            ...
            steps = epochs * B
        ### the attn weights of all steps/epochs along the training process are saved
